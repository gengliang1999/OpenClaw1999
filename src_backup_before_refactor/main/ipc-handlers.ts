    ...
    try {
      ...
      if (url === '/system/parseDocument' && method === 'POST') {
        ...
        try {
          ...
          // 2. 预先做父子切分
          const chunks = DocumentParser.splitTextIntoParentChild(parsedText);

          // 3. 写入内存 RAG 引擎以供即时查询
          const { ragEngine } = require('../backend/rag-engine');
          ragEngine.addDocument(convId, fileName, chunks); // 传递 chunks 数组以录入父子映射

          // 临时上下文仅存放于内存，不再执行强制持久化任务，防止污染知识库
