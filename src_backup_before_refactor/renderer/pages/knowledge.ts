...
function bindEvents(container: HTMLElement) {
  ...
  const fileInput = container.querySelector('#fileUploadInput') as HTMLInputElement;
  const listContainer = container.querySelector('#listContainer') as HTMLElement;
  const addMoreBtn = container.querySelector('#addMoreBtn') as HTMLButtonElement;
  const dataList = container.querySelector('#dataList') as HTMLUListElement;
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      activeTab = (tab as HTMLElement).dataset.tab || 'verified';
      loadTabData();
    });
  });
  ...
  async function loadTabData() {
    ...
    try {
      const backupActions = container.querySelector('#knowledgeBackupActions') as HTMLElement;
      if (activeTab === 'verified') {
        dropZone.style.display = 'none';
        listContainer.style.display = 'flex';
        addMoreBtn.style.display = 'block';
        if (backupActions) backupActions.style.display = 'flex';
  ...
  // 渲染已入库列表
  function renderVerifiedFiles(files: any[]) {
    dataList.innerHTML = '';
    files.forEach(f => {
      ...
      const editBtn = li.querySelector('.edit-file-btn') as HTMLButtonElement;
      ...
      const deleteBtn = li.querySelector('.delete-file-btn') as HTMLButtonElement;
  ...
  // 渲染暂存区列表
  function renderStagingItems(items: any[]) {
    dataList.innerHTML = '';
    ...
    items.forEach(item => {
      ...
      // 绑定批准按钮
      const approveBtn = li.querySelector('.stage-approve-btn') as HTMLButtonElement;
      ...
      // 绑定丢弃按钮
      const rejectBtn = li.querySelector('.stage-reject-btn') as HTMLButtonElement;
  ...
  function renderJobs(jobs: any[]) {
    ...
    jobs.forEach(job => {
      ...
      if (job.task_type === 'memory-extraction') typeName = '🧠 长期记忆原子提取';
      else if (job.task_type === 'memory-consolidation') typeName = '🔄 SQLite 记忆反刍整理';
      else if (job.task_type === 'url') typeName = '🕸️ 采集源网页爬取鉴伪';
  ...
  async function handleFiles(files: File[]) {
    ...
    try {
      ...
      // 成功投递后，强制刷新并展示已入库列表
      activeTab = 'verified';
      const verifiedTab = container.querySelector('[data-tab="verified"]') as HTMLElement;
      if (verifiedTab) {
        verifiedTab.click();
      } else {
        loadTabData();
  ...
  const searchQuery = container.querySelector('#searchQuery') as HTMLTextAreaElement;
  const searchResults = container.querySelector('#searchResults') as HTMLElement;
  ...
  // 绑定批量导入与导出物理备份按钮事件
  const importKnowledgeBtn = container.querySelector('#importKnowledgeBtn') as HTMLButtonElement;
  const exportKnowledgeBtn = container.querySelector('#exportKnowledgeBtn') as HTMLButtonElement;
    ...
    importKnowledgeBtn.addEventListener('click', async () => {
      try {
        ...
        if (res && res.success) {
          ...
          const jobsTab = container.querySelector('.sidebar-tab[data-tab="jobs"]') as HTMLElement;
...
// 物理归档文档整文编辑弹窗
function showEditKnowledgeModal(source: string, category: string, onSaved: () => void) {
  ...
  function openEditor(originalText: string) {
    ...
    const textarea = modal.querySelector('#editKnowledgeTextarea') as HTMLTextAreaElement;
    textarea.value = originalText;

    const cancelBtn = modal.querySelector('#cancelEditKnowledgeBtn') as HTMLButtonElement;
    const saveBtn = modal.querySelector('#saveEditKnowledgeBtn') as HTMLButtonElement;
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        modal.remove();
        if (window.__toast) window.__toast.success('文档已更新，后台重新提炼中！');
        else alert('文档更新成功，后台已启动向量切分重算！');
        onSaved();
      } catch (e: any) {
