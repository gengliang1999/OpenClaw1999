...
class MemoryStore {
  ...
  async init() {
    ...
    try { this.db.run('ALTER TABLE memories ADD COLUMN recall_count INTEGER DEFAULT 0'); } catch(e) {}
    try { this.db.run('ALTER TABLE memories ADD COLUMN last_recalled_at TEXT'); } catch(e) {}
    try { this.db.run('ALTER TABLE memories ADD COLUMN is_pinned INTEGER DEFAULT 0'); } catch(e) {}
    ...
    return values.map(row => {
      ...
      columns.forEach((col, i) => {
        if (col === 'tags') {
          ...
          try { obj[col] = JSON.parse(row[i]); } catch { obj[col] = row[i]; }
        } else {
          obj[col] = row[i];
        }
      });
  ...
  addEntityRelation(subject, predicate, object, sourceMemoryId = null) {
    ...
    const now = new Date().toISOString();
    this.db.run(
      'INSERT INTO entity_relationships (id, subject, predicate, object, source_memory_id, created_at) VALUES (?, ?, ?, ?, ?, ?)',
      [id, subject, predicate, object, sourceMemoryId, now]
    );
    this._debouncedSave();
  ...
  getPendingJobs() {
    const results = this.db.exec(
      "SELECT * FROM background_jobs WHERE status = 'pending' OR status = 'retry' ORDER BY created_at ASC"
    );
    return this._parseResults(results);
  }
  ...
  updateJobStatus(id, status, error = null, retryCount = 0) {
    const now = new Date().toISOString();
    this.db.run(
      'UPDATE background_jobs SET status = ?, last_error = ?, retry_count = ?, updated_at = ? WHERE id = ?',
      [status, error, retryCount, now, id]
    );
    this._debouncedSave();
  }
