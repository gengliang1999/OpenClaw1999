...
class MockModelManager {
  async chatStream(messages: any[], options: any, onChunk: (chunk: string) => void) {
    onChunk(`正在思考...`);
    let full = '';
...
// Mock Sandbox
class MockSandbox {
  async execute(cmd: string, options: any) {
...
async function runE2ETest() {
  ...
  class MockErrorEvolutionEngine {
    async evolve(errorCtx: string, taskGoal: string) {
      return `> 🧬 **[系统进化完成]** 测试: 我已学习如何处理 "${taskGoal}" 的失败: "${errorCtx}"，固化技能成功。`;
    }
  }

  const agentLoopError = new AgentLoop({
