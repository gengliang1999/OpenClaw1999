...
class MemoryEngine {
    ...
    getVectorDbPath(filename = 'memories_vectors.json') {
     ...
     */
    async reconcileAndStore(factInput, category) {
    ...
    async unifiedRetrieval(query, knowledgeCategory = '默认知识库') {
        ...
        if (!queryEmbedding) {
            ...
            try {
                ...
                for (const sq of searchQueries) {
                    const resultsEp = this.memoryStore.db.exec('SELECT * FROM memory_episodes WHERE summary LIKE ? OR topics_json LIKE ? ORDER BY created_at DESC LIMIT 3', [`%${sq}%`, `%${sq}%`]);
        ...
        tasks.push((async () => {
            try {
                ...
                if (!fs.existsSync(promotedPath))
                    return;
                await vectorDbManager.executeRead(promotedPath, async (promotedStore) => {
