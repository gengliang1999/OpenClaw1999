 ...
 */
class AgentLoop extends events_1.EventEmitter {
    deps;
    maxRecursion = 3;
    constructor(deps) {
        super();
    ...
    async run(context) {
        ...
        while (loopCount < this.maxRecursion) {
            ...
            const fullReply = await modelManager.chatStream(context.messages, { modelId: context.modelId, temperature: context.temperature, signal: context.signal, agentMode: true }, (chunk) => {
            ...
            if (!cmdToExecute) {
                ...
                if (toolMatch) {
                    try {
                        payload = JSON.parse(toolMatch[1]);
                    }
                    catch (e) { }
                }
                else if (rawJsonMatch) {
                    try {
                        payload = JSON.parse(rawJsonMatch[1]);
                    }
                    catch (e) { }
                }
                if (payload) {
