...
class ContextAggregator {
    ...
    async executeChatStream(payload, signal) {
        ...
        if (history.length > MAX_RECENT) {
            ...
            const olderTranscript = olderMessages
                .map((m) => `${m.role === 'user' ? '用户' : 'AI'}：${(typeof m.content === 'string' ? m.content : '[多模态内容]').slice(0, 150)}`)
                .join('\n');
        ...
        let messages = history.map((m, index) => {
            ...
            if (parsed) {
                if (index < history.length - 1) {
                    const textBlock = parsed.find((c) => c.type === 'text');
        ...
        // 4. 驱动 Agent 思考与流式输出
        const evolutionEngine = new evolution_engine_1.EvolutionEngine(dataDir, memoryStore, modelManager);
