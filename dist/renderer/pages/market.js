...
export async function render(container, params = {}) {
    ...
    try {
        const am = await api.model.getActiveModel();
        activeModelId = am?.id || '';
    }
    catch (e) { }
  ...
  `;
    ...
    if (params && params.openConfig) {
        ...
        let v = cloudVendors.find(x => x.id.toLowerCase() === targetId ||
...
function bindModalEvents() {
    ...
    document.getElementById('closeCloudConfig')?.addEventListener('click', closeCloudModal);
    document.getElementById('cancelCloudConfig')?.addEventListener('click', closeCloudModal);
    ccm?.addEventListener('mousedown', (e) => { if (e.target === ccm)
        closeCloudModal(); });
    // 自定义模型按钮
    ...
    grid.querySelectorAll('.cloud-vendor-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (isExisting) {
                ...
                if (v)
                    openCloudConfig({ ...v, isExisting: false });
            }
        });
    });
}
...
async function saveCloudConfig(vendor) {
    ...
    if (!apiKey && !baseUrl) {
        window.__toast?.error('请填写 API 地址或 API Key');
        return;
    }
    if (!selectedModel) {
        window.__toast?.error('请选择或手动输入一个模型');
        return;
    }
    ...
    try {
        ...
        // 持久化到存储
        try {
            await api.settings.set(settingsKey, config);
        }
        catch (e) {
            console.warn('settings.set 失败:', e);
        }
        window.__toast?.success(`${customName} 配置已保存！`);
        const cModal = document.getElementById('cloudConfigModal');
        if (cModal) {
...
async function fetchModelsFromApi() {
    ...
    if (!baseUrl) {
        window.__toast?.error('请先填写 API 地址');
        return;
    }
    btn.disabled = true;
    btn.textContent = '⏳ 拉取中...';
    ...
    `;
        actionsEl.querySelector('.local-download-btn').addEventListener('click', (e) => {
            ...
            if (window.openClaw?.system?.openExternal)
                window.openClaw.system.openExternal(url);
            else
...
async function openLocalModelsModal(provider) {
  ...
  `;
    ...
    batchBtn.addEventListener('click', async () => {
        ...
        if (checked.length === 0)
            return;
        if (!confirm(`确定要删除 ${checked.length} 个模型吗？此操作不可恢复。`))
            return;
        ...
        for (const cb of checked) {
            try {
                ...
                fail++;
                console.warn('删除失败:', cb.dataset.id, e);
            }
        }
        if (success > 0)
            window.__toast?.success(`已删除 ${success} 个模型`);
        if (fail > 0)
            window.__toast?.error(`${fail} 个模型删除失败`);
