...
export async function render(container) {
  ...
  `;
    logContainer = document.getElementById('cm-log-container');
    statusDot = document.getElementById('cm-status-dot');
    statusText = document.getElementById('cm-status-text');
    // 绑定按钮事件
    ...
    document.getElementById('btn-bind')?.addEventListener('click', async () => {
        try {
            if (!window.openClaw?.system?.selectDirectory)
                return;
            ...
            if (res && res.message) {
                if (window.__toast)
                    window.__toast.success(res.message);
                appendLog(`> ✅ 挂载成功！`);
                fetchStatus();
            }
        }
    ...
    if (window.openClaw && window.openClaw.onCoreManagerLog) {
        window.openClaw.onCoreManagerLog((msg) => {
            appendLog(msg);
        });
    }
    // 初始获取并定期轮询状态
...
async function fetchStatus() {
    try {
        ...
        const { installStatus, daemonStatus, currentDir } = res;
...
function appendLog(msg) {
    ...
    // 简单的控制台色彩解析 (红黄绿)
    let color = '#58a6ff';
    if (msg.includes('ERROR') || msg.includes('❌') || msg.includes('🚨'))
        color = '#ff7b72';
