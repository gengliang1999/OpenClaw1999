...
export async function render(container) {
  ...
  `;
    ...
    document.getElementById('batchReinforceBtn').addEventListener('click', async () => {
        if (memories.length === 0) {
            if (window.__toast)
                window.__toast.info('当前暂无记忆可以复习');
            return;
        }
        if (await asyncConfirm(`确定要依据艾宾浩斯遗忘曲线一键复习当前 ${memories.length} 条记忆吗？保留率将重置提升至 100%！`)) {
            try {
                ...
                if (window.__toast)
                    window.__toast.success('全量记忆复习成功！保留率已全部提升为 100%');
                else
                    alert('全量记忆复习成功！');
                loadData();
            }
                ...
                if (window.__toast)
                    window.__toast.error('全量复习失败: ' + e.message);
                else
                    alert('全量复习失败: ' + e.message);
            }
        }
    });
...
async function loadData() {
    ...
    try {
        ...
        if (avgEl)
            avgEl.textContent = `${avgRet}%`;
        if (freshEl)
            freshEl.textContent = String(freshCount);
        if (fadingEl)
            fadingEl.textContent = String(fadingCount);
        renderList();
    }
    catch (e) {
...
function parseTagsHelper(tagsInput) {
    if (!tagsInput)
        return [];
...
function calculateEbbinghausRetention(createdAt, isPinned, isPromoted, reinforceCount = 0) {
...
function generateEbbinghausSparkline(retention, color, id) {
    const width = 180;
    const height = 22;
...
function renderList() {
    ...
    // 绑定完全兼容 CSP 的展开折叠处理
    container.querySelectorAll('.memory-card-node').forEach((card) => {
...
// 艾宾浩斯曲线记忆复习刷新
window._reinforceMemory = async (id) => {
    const memory = memories.find((m) => m.id === id);
    try {
        ...
        if (window.__toast)
            window.__toast.success('已依据艾宾浩斯遗忘曲线成功复习此记忆，保留率提升至 100%！');
        else
            alert('记忆复习成功！保留率已恢复至 100%');
        loadData();
    }
    catch (e) {
        if (window.__toast)
            window.__toast.error('复习失败: ' + e.message);
        else
            alert('复习失败: ' + e.message);
    }
};
// 记忆晋升二段式事件
