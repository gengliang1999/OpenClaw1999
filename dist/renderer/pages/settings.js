...
export async function render(container) {
  ...
  `;
    // 绑定事件
    const changeDataDirBtn = document.getElementById('changeDataDirBtn');
    ...
    if (changeDataDirBtn) {
        changeDataDirBtn.addEventListener('click', async () => {
            try {
                ...
                if (window.__toast)
                    window.__toast.error('保存用户数据目录失败: ' + (e.message || '未知错误'));
            }
        });
    }
    if (changeDownloadDirBtn) {
        changeDownloadDirBtn.addEventListener('click', async () => {
            try {
                ...
                if (window.__toast)
                    window.__toast.error('保存下载目录失败: ' + (e.message || '未知错误'));
            }
        });
    }
    if (changeLogDirBtn) {
        changeLogDirBtn.addEventListener('click', async () => {
            try {
                ...
                if (window.__toast)
                    window.__toast.error('保存日志目录失败: ' + (e.message || '未知错误'));
            }
        });
    }
    const changeMemoryDbBtn = document.getElementById('changeMemoryDbBtn');
        ...
        changeMemoryDbBtn.addEventListener('click', async () => {
            try {
                ...
                if (result && result.filePath) {
                    if (window.__toast)
                        window.__toast.success('记忆数据库路径已保存，重启后生效');
                    // 即时刷新页面显示
                    render(container);
                }
            }
                ...
                if (window.__toast)
                    window.__toast.error('修改记忆保存路径失败: ' + (e.message || '未知错误'));
            }
        });
    }
}
