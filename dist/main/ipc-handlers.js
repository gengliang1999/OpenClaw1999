...
function registerApiIpc(dependencies, mainWindowRef, expectedToken) {
        ...
        try {
            ...
            if (url === '/system/global-config') {
                if (method === 'GET') {
                    ...
                    if (customMemoryDbPath !== undefined)
                        config.customMemoryDbPath = customMemoryDbPath;
            ...
            if (url === '/system/memory/select-db' && method === 'POST') {
                ...
                if (!recent.includes(newPath)) {
                    ...
                    recent = recent.filter((p) => p !== newPath);
                    recent.unshift(newPath);
                }
            ...
            if (url === '/system/memory/switch-db' && method === 'POST') {
                ...
                if (!recent.includes(dbPath)) {
                    ...
                    recent = recent.filter((p) => p !== dbPath);
                    recent.unshift(dbPath);
                }
                config.recentMemoryDbs = recent.slice(0, 5);
                fs.writeFileSync(globalConfigPath, JSON.stringify(config, null, 2), 'utf8');
            ...
            if (url === '/system/memory/rename-db' && method === 'POST') {
                ...
                let recent = config.recentMemoryDbs || [];
                recent = recent.map((p) => p === oldPath ? newDbPath : p);
                config.recentMemoryDbs = recent.filter((p) => fs.existsSync(p));
                fs.writeFileSync(globalConfigPath, JSON.stringify(config, null, 2), 'utf8');
            ...
            if (url === '/chat/conversations') {
                if (method === 'GET')
                    return memoryStore.getConversations();
                if (method === 'POST') {
            ...
            if (url === '/memory/promote/generate' && method === 'POST') {
                ...
                if (!memoryId)
                    throw new Error('memoryId 不能为空');
                // 1. 获取零碎记忆内容
                const memory = memoryStore.getMemory(memoryId);
                if (!memory)
                    throw new Error('未找到对应的记忆条目');
            ...
            if (url === '/memory/promote/confirm' && method === 'POST') {
                ...
                memoryStore.db.run('UPDATE memories SET tags = ?, updated_at = ? WHERE id = ?', [JSON.stringify(currentTags), new Date().toISOString(), memoryId]);
                memoryStore._save();
                return {
                    success: true,
                    fileName,
            ...
            if (url.startsWith('/memory')) {
                ...
                if (url === '/memory/export' && method === 'POST') {
                    ...
                    try {
                        await vectorDbManager.executeRead(currentMemoriesVectorsPath, async (store) => {
                ...
                if (url === '/memory/import' && method === 'POST') {
                    ...
                    if (overwrite) {
                        ...
                        memoryStore._save();
                        await vectorDbManager.executeWrite(currentMemoriesVectorsPath, async (store) => {
                            store.documents = [];
                        }, false);
                    }
                    const list = memories;
                        ...
                        await vectorDbManager.executeWrite(currentMemoriesVectorsPath, async (store) => {
                            if (overwrite) {
                                ...
                                for (const vec of vectors) {
                                    const checkExist = store.documents.some((doc) => doc.content === vec.content);
                                    if (!checkExist) {
                ...
                if (url.startsWith('/memory/') && !url.endsWith('/pin') && method === 'PUT') {
                    ...
                    setImmediate(async () => {
                        try {
                            ...
                            await vectorDbManager.executeWrite(vectorPath, async (store) => {
                ...
                if (url.startsWith('/memory/') && method === 'DELETE') {
                    ...
                    if (memory) {
                        try {
                            await vectorDbManager.executeWrite(path.join(memoryStore.dataDir, 'memories_vectors.json'), async (store) => {
                ...
                if (url.startsWith('/memory/') && url.endsWith('/pin') && method === 'PUT') {
                    ...
                    memoryStore.db.run('UPDATE memories SET is_pinned = ?, updated_at = ? WHERE id = ?', [pinValue, new Date().toISOString(), id]);
                    memoryStore._save();
                ...
                if (url === '/memory/promote/confirm' && method === 'POST') {
                    ...
                    if (!memoryId)
                        throw new Error('memoryId 不能为空');
                    if (!draftMarkdown || !draftMarkdown.trim())
                    ...
                    try {
                        ...
                        if (Array.isArray(embedding) && embedding.length > 0) {
                            await vectorDbManager.executeWrite(promotedPath, async (store) => {
                                await store.addDocuments([{
                                        id: require('crypto').randomUUID(),
                    ...
                    if (!tagsArray.includes('promoted')) {
                        ...
                        memoryStore.db.run('UPDATE memories SET tags = ?, updated_at = ? WHERE id = ?', [JSON.stringify(tagsArray), new Date().toISOString(), memoryId]);
                        memoryStore._save();
                    }
                    return { success: true };
                }
            ...
            if (url === '/knowledge/ingest-url' && method === 'POST') {
                ...
                if (!targetUrl)
                    throw new Error('超链接 URL 不能为空');
                if (!convId)
                    throw new Error('必须关联会话 ID');
            ...
            if (url === '/knowledge/sources') {
                ...
                if (!settings.crawlingSources)
                    settings.crawlingSources = [];
                if (method === 'GET') {
                    ...
                    if (!sourceUrl)
                        throw new Error('源地址不能为空');
                    settings.crawlingSources = settings.crawlingSources.filter((s) => s !== sourceUrl);
                    fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), 'utf8');
                    return { success: true, sources: settings.crawlingSources };
                }
            }
            if (url.startsWith('/knowledge/files')) {
                ...
                if (method === 'GET') {
                    return await vectorDbManager.executeRead(dbPath, (store) => {
                        const fileMap = {};
                        // @ts-ignore
                    ...
                    const { source } = body;
                    if (!source)
                        throw new Error('必须指定要删除的来源名称');
                    await vectorDbManager.executeWrite(dbPath, async (store) => {
                        // 联动还原：若该知识文件由记忆晋升而来，删除文件时应恢复其在 SQLite 中的原始记忆活力
                        try {
                            // @ts-ignore
                            const docsToDelete = store.documents.filter((d) => d.metadata && d.metadata.source === source);
                            const sourceMemoryId = docsToDelete.find((d) => d.metadata && d.metadata.sourceMemoryId)?.metadata?.sourceMemoryId;
                            if (sourceMemoryId) {
                                ...
                                if (mem) {
                                    ...
                                    tagsArray = tagsArray.filter((t) => t !== 'promoted');
                                    // [Red Team Fix] 赋予被剥夺 promoted 资格的记忆满级唤醒复活无敌帧，避免立刻被艾宾浩斯判定过滤
                                    const newRecallCount = (mem.recall_count || 0) + 1;
                                    memoryStore.db.run('UPDATE memories SET tags = ?, updated_at = ?, last_recalled_at = ?, recall_count = ? WHERE id = ?', [JSON.stringify(tagsArray), new Date().toISOString(), new Date().toISOString(), newRecallCount, sourceMemoryId]);
                                    memoryStore._save();
            ...
            if (url.startsWith('/knowledge/staging')) {
                ...
                if (url.startsWith('/knowledge/staging/action') && method === 'POST') {
                    ...
                    if (action === 'reject') {
                        ...
                        await vectorDbManager.executeWrite(stagingPath, async (stagingStore) => {
                            // @ts-ignore
                            docsToMove = stagingStore.documents.filter((doc) => doc.metadata?.source === source);
                        ...
                        await vectorDbManager.executeWrite(mainPath, async (mainStore) => {
                            const verifiedDocs = docsToMove.map((doc) => {
                ...
                if (method === 'GET') {
                    return await vectorDbManager.executeRead(stagingPath, (stagingStore) => {
                        const stagingMap = {};
                        // @ts-ignore
            ...
            if (url === '/knowledge/watched-folders') {
                ...
                if (method === 'GET') {
                    ...
                    if (!folderPath || !category)
                        throw new Error('监控路径和目标分类不能为空');
                    const exists = settings.watchedFolders.some((f) => f.path === folderPath);
                    if (!exists) {
                    ...
                    if (!folderPath)
                        throw new Error('监控路径不能为空');
                    settings.watchedFolders = settings.watchedFolders.filter((f) => f.path !== folderPath);
                    fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), 'utf8');
                    // 热重载
                    folderWatcher.startWatching(settings.watchedFolders);
                    return { success: true, watchedFolders: settings.watchedFolders };
            ...
            if (url === '/models/active') {
                if (method === 'GET')
                    return { modelId: modelManager.activeModelId };
                if (method === 'PUT') {
            ...
            if (url === '/models/local/add') {
                ...
                let existing = modelManager.models.find((m) => m.id === globalId);
                let newModel;
                if (!existing) {
                    const config = {
                        id: globalId,
                    ...
                    if (imageExts.includes(ext)) {
                        ...
                        if (officeExts.includes(ext)) {
                            ...
                            await DocumentParser.parseTextFileStream(filePath, async (chunks) => {
                                const chunkText = chunks.map((c) => c.parentContent).join('\n\n');
            ...
            if (url.startsWith('/skills')) {
                ...
                try {
                    if (fs.existsSync(skillsPath))
                        installedSkills = JSON.parse(fs.readFileSync(skillsPath, 'utf8'));
                }
                catch (e) { }
                ...
                if (url.startsWith('/skills/marketplace')) {
                    ...
                    if (sortBy === 'updated')
                        skills.sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated));
            ...
            if (url.startsWith('/plugins')) {
                ...
                try {
                    if (fs.existsSync(pluginsPath))
                        installedPlugins = JSON.parse(fs.readFileSync(pluginsPath, 'utf8'));
                }
                catch (e) { }
                ...
                if (url.startsWith('/plugins/marketplace')) {
                    ...
                    const allTags = OPENHUB_REGISTRY ? [...new Set(OPENHUB_REGISTRY.flatMap((pl) => pl.tags || []))] : [];
                    const allTypes = OPENHUB_REGISTRY ? [...new Set(OPENHUB_REGISTRY.map((pl) => pl.type))] : [];
                    return {
                        source: 'OpenHub',
                        total,
                        page,
            ...
            if (url === '/core-manager/status') {
                const installStatus = openclaw_installer_1.openClawInstaller.getStatus();
                const daemonStatus = openclaw_daemon_1.openClawDaemon.getStatus();
                // 加入当前配置的路径给前端展示
                return { installStatus, daemonStatus, currentDir: openclaw_installer_1.openClawInstaller.getInstallDir() };
            }
            if (url === '/core-manager/bind-path' && method === 'POST') {
                const { targetPath } = body;
                if (!targetPath)
                    throw new Error('目标路径为空');
                openclaw_installer_1.openClawInstaller.setCustomDir(targetPath);
                openclaw_daemon_1.openClawDaemon.setInstallDir(targetPath);
            ...
            if (url.startsWith('/core-manager/action/')) {
                ...
                if (action === 'install') {
                    openclaw_installer_1.openClawInstaller.install(onLog).catch(e => onLog(`[ERROR] ${e.message}`));
                    return { success: true, message: '安装进程已转入后台流水线执行' };
                }
                else if (action === 'update') {
                    openclaw_installer_1.openClawInstaller.update(onLog).catch(e => onLog(`[ERROR] ${e.message}`));
                    return { success: true, message: '更新进程已在后台执行' };
                }
                else if (action === 'uninstall') {
                    openclaw_installer_1.openClawInstaller.uninstall(onLog).catch(e => onLog(`[ERROR] ${e.message}`));
                    return { success: true, message: '卸载进程已在后台执行' };
                }
                else if (action === 'start') {
                    openclaw_daemon_1.openClawDaemon.start(onLog).catch(e => onLog(`[ERROR] ${e.message}`));
                    return { success: true, message: '拉起进程指令已下达' };
                }
                else if (action === 'stop') {
                    openclaw_daemon_1.openClawDaemon.stop(onLog);
            ...
            if (url === '/permissions') {
                if (method === 'GET')
                    return permissionManager.getPermissionConfig();
                if (method === 'PUT') {
    ...
    electron_1.ipcMain.handle('api:chat:stream', (event, payload) => {
        const { conversationId } = payload;
        const mainWindow = mainWindowRef();
        if (!mainWindow)
            return { started: false, error: '主窗口未就绪' };
    ...
    // 主动中断对话流
    electron_1.ipcMain.handle('api:chat:abort', (event, payload) => {
        const { conversationId } = payload || {};
        if (conversationId) {
    ...
    // 流式提示词优化句柄
    electron_1.ipcMain.handle('api:chat:optimize-stream', async (event, payload) => {
