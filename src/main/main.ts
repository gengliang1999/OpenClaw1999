...
app.whenReady().then(async () => {
  ...
  // 注册大模型异步记忆抽取任务处理器
  jobWorker.registerHandler('memory-extraction', async (payload: any) => {
  ...
  jobWorker.registerHandler('memory-vectorization', async (payload: any) => {
    ...
    if (Array.isArray(embedding) && embedding.length > 0) {
      await vectorDbManager.executeWrite(memoriesVectorsPath, async (store: any) => {
