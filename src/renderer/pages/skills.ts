...
export async function render(container) {
  ...
  (document.getElementById('tabSkillInstalled') as any).addEventListener('click', (e) => {
    switchTab('installed', (e.target as any));
  });
...
window._installSkill = async (id) => {
  try {
    if(window.__toast) window.__toast.info('正在载入技能突触...');
    await api.skill.installSkill(id);
    if(window.__toast) window.__toast.success('节点载入成功');
    await loadData();
    renderList('market');
  } catch (e) {
    if(window.__toast) window.__toast.error('载入失败: ' + e.message);
  }
};
...
window._uninstallSkill = async (id) => {
  try {
    ...
    if(window.__toast) window.__toast.success('技能节点已剥离');
    await loadData();
    renderList('installed');
  } catch (e) {
    if(window.__toast) window.__toast.error('剥离失败: ' + e.message);
  }
};
