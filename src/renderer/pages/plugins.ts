...
export async function render(container) {
  ...
  (document.getElementById('tabInstalled') as any).addEventListener('click', (e) => {
    switchTab('installed', (e.target as any));
  });
  (document.getElementById('tabMarket') as any).addEventListener('click', (e) => {
...
function renderList(tab) {
  const container = (document.getElementById('pluginContent') as any);
...
window._installPlugin = async (id) => {
  try {
    ...
    if(window.__toast) window.__toast.error('安装失败: ' + e.message);
  }
};
...
window._uninstallPlugin = async (id) => {
  try {
    ...
    if(window.__toast) window.__toast.error('卸载失败: ' + e.message);
  }
};
