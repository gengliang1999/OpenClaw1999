...
export async function init(container) {
  ...
  // 搜索
  (document.getElementById('searchExpert') as any).addEventListener('input', debounce((e) => {
    currentSearch = (e.target as any).value.toLowerCase();
    renderExpertsGrid(currentCat, currentSearch, combinedExperts);
  }, 300));

  // 药丸标签分类切换
  const categories = (document.getElementById('expertCategories') as any);
  if (categories) {
    categories.addEventListener('click', (e) => {
      const btn = (e.target as any).closest('.expert-cat-tab');
      if (!btn) return;
...
function renderExpertsGrid(category, search, combinedExperts = EXPERTS) {
  const grid = (document.getElementById('expertsGrid') as any);
