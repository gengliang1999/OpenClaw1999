...
export async function render(container) {
  ...
  (document.getElementById('optimizePromptBtn') as any).addEventListener('click', async () => {
    ...
    if (sendBtn) sendBtn.disabled = true; // 禁用发送按钮，防止发送未完成内容
    ...
    try {
      ...
      input.focus();
      if (window.__toast) window.__toast.success('✨ 提示词智能优化完成！');
    } catch (err: any) {
      ...
      if (sendBtn) sendBtn.disabled = false; // 恢复发送按钮
    }
  });
...
async function loadHistory(convId) {
  try {
    ...
    if (c && titleEl) {
      (titleEl as any).textContent = c.title || '新对话';
    }
    ...
    if (state && state.isGenerating) {
      ...
      if (container) {
        ...
        if (msgs && msgs.length > 0 && msgs[msgs.length - 1].role === 'assistant' && lastAssistantBox) {
          ...
          state.timerId = startThinkingTimer(lastAssistantBox as HTMLElement, state.startTime, state);
        } else {
