...
async function withTimeout(promise: Promise<any>, timeoutMs = TIMEOUT_LIMIT): Promise<any> {
  let timer: any;
...
export const api = {
  get: async (url: string, options: any = {}): Promise<any> => {
    if (window.openClaw && window.openClaw.apiCall) {
      ...
      return withTimeout(window.openClaw.apiCall(url, { ...options, method: options.method || 'GET' }) as Promise<any>);
    }
  ...
  chat: {
    ...
    sendMessageStream: (conversationId: string, message: string, attachment: any, modelId: string, systemPrompt: string, temperature: number, agentMode: any, onData: (data: any) => void) => {
      if (onData) {
      ...
      if (window.openClaw && window.openClaw.apiCallStream) {
        ...
        window.openClaw.apiCallStream({ conversationId, message, attachment, modelId, systemPrompt, temperature, agentMode }).catch((err: any) => {
          ...
          if (onData) onData({ type: 'error', message: err.message, conversationId });
          streamCallbacks.delete(conversationId);
        });
      }
