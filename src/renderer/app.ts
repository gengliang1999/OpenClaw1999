...
function bindGlobalShortcuts() {
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      // 聊天页弹窗
      const modelSelectionModal = (document.getElementById('modelSelectionModal') as any);
      if (modelSelectionModal && modelSelectionModal.style.display === 'flex') {
        modelSelectionModal.style.display = 'none';
        return;
      }

      const attachmentPreview = (document.getElementById('attachmentPreview') as any);
      if (attachmentPreview && attachmentPreview.style.display === 'flex') {
        (document.getElementById('removeAttachmentBtn') as any)?.click();
        return;
      }

      const quotePreview = (document.getElementById('quotePreview') as any);
      if (quotePreview && quotePreview.style.display === 'flex') {
        (document.getElementById('closeQuoteBtn') as any)?.click();
        return;
      }

      // 市场页弹窗
      const cloudConfigModal = (document.getElementById('cloudConfigModal') as any);
      if (cloudConfigModal && cloudConfigModal.classList.contains('visible')) {
        cloudConfigModal.classList.remove('visible');
        return;
      }

      const installGuideModal = (document.getElementById('installGuideModal') as any);
      if (installGuideModal && installGuideModal.style.display === 'flex') {
        installGuideModal.style.display = 'none';
        return;
      }

      const localModelsModal = (document.getElementById('localModelsModal') as any);
      ...
      if (renameInputs.length > 0) {
        (document.getElementById('renameCancel') as any)?.click();
        return;
      }
    }
  });
...
function renderShell() {
  ...
  // 搜索会话
  (document.getElementById('sidebarConvSearch') as any)?.addEventListener('input', (e) => {
    // 搜索时清空批量选择，避免隐藏项被误删
    if (batchMode) {
      batchSelected.clear();
    }
    loadSidebarConversations((e.target as any).value.trim());
  });

  // 新建对话按钮
  (document.getElementById('sidebarNewChatBtn') as any)?.addEventListener('click', async () => {
  ...
  // 垃圾篓按钮
  (document.getElementById('sidebarTrashBtn') as any)?.addEventListener('click', () => {
    showTrashPanel();
  });
...
function initLogoUpload() {
  const logoEl = (document.getElementById('sidebarLogo') as any);
  const uploadInput = (document.getElementById('logoUploadInput') as any);
  const logoIcon = (document.getElementById('logoIcon') as any);
  ...
  logoEl.addEventListener('click', (e) => {
    // 如果右键菜单打开则不触发
    if ((document.querySelector('.conv-context-menu') as any)) return;
    uploadInput.click();
  });
  ...
  logoEl.addEventListener('contextmenu', (e) => {
    ...
    if (localStorage.getItem('openclaw_custom_logo')) {
      ...
      const restoreItem = (document.createElement('div') as any);
...
async function loadSidebarConversations(query = '') {
  ...
  const listEl = (document.getElementById('sidebarConvList') as any);
  if (!listEl) return;
  ...
  listEl.querySelectorAll('.sidebar-conv-item').forEach(el => {
    el.addEventListener('click', async (e) => {
      if ((e.target as any).closest('.batch-conv-checkbox')) return;
      if (batchMode) return;
...
function showConvContextMenu(convId, x, y) {
  ...
  items.forEach(item => {
    ...
    const row = (document.createElement('div') as any);
...
async function renameConversationUI(convId) {
  ...
  const input = (document.getElementById('renameInput') as any);
  // 获取当前标题
  try {
  ...
  return new Promise(resolve => {
    ...
    (document.getElementById('renameCancel') as any).onclick = close;
    overlay.onclick = (e) => { if ((e.target as any) === overlay) close(); };
    (document.getElementById('renameConfirm') as any).onclick = async () => {
    ...
    input.onkeydown = (e) => { if (e.key === 'Enter') (document.getElementById('renameConfirm') as any).click(); };
  });
}
...
async function exportConversationUI(convId) {
  ...
  (document.getElementById('closeExportBtn') as any).onclick = close;
  overlay.onclick = (e) => { if ((e.target as any) === overlay) close(); };
...
function loadExternalScript(url) {
  return new Promise((resolve, reject) => {
    // 检查是否已加载
    const existing = (document.querySelector(`script[src="${url}"]`) as any);
    if (existing) {
...
async function updateTrashBadge() {
  try {
    ...
    const btn = (document.getElementById('sidebarTrashBtn') as any);
...
async function showTrashPanel() {
  ...
  const close = () => { overlay.style.opacity = '0'; setTimeout(() => overlay.remove(), 200); };
  (document.getElementById('closeTrashBtn') as any).onclick = close;
  overlay.onclick = (e) => { if ((e.target as any) === overlay) close(); };
...
function bindRouteEvents() {
  ...
  sidebar.addEventListener('click', (e) => {
    // 收缩/展开按钮点击
    if ((e.target as any).closest('#sidebarBottomToggleBtn')) {
    ...
    const item = (e.target as any).closest('.sidebar-nav-item');
    if (!item) return;
  ...
  // 侧边栏拖动调整宽度逻辑
  const resizer = (document.getElementById('sidebarResizer') as any);
...
async function navigateTo(route, params = {}) {
  ...
  try {
    ...
    // 创建页面容器
    const pageEl = (document.createElement('div') as any);
...
export function setModelName(name) {
  currentModel = name;
  const el = (document.getElementById('modelName') as any);
...
document.addEventListener('click', (e) => {
  ...
  if (globalModalIds.includes((e.target as any).id) || (e.target as any).classList.contains('cloud-config-modal')) {
    (e.target as any).style.display = 'none';
    (e.target as any).classList.remove('visible');
  }
});
