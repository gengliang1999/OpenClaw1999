...
export class MemoryEngine {
   ...
   */
  extractMemoriesFromRegex(reply: string): Array<{ fact: string, category: string }> {
    const results: Array<{ fact: string, category: string }> = [];
  ...
  async reconcileAndStore(fact: string, category: string): Promise<string> {
    ...
    try {
      ...
      if (embedding && Array.isArray(embedding) && embedding.length === EXPECTED_EMBEDDING_DIM) {
        await vectorDbManager.executeRead(dbPath, async (memVecStore: any) => {
    ...
    if (!topMatch) {
      ...
      await vectorDbManager.executeWrite(dbPath, async (memVecStore: any) => {
...
格式：{"action": "CONTRADICT" 或 "APPEND" 或 "IGNORE", "merged_fact": "合并后的新事实（仅 CONTRADICT 时需要）", "reason": "简短理由"}`
  ...
  private async _vectorizeMemory(vecStore: any, fact: string, memoryId: string) {
    try {
      const embedding = await this.modelManager.getEmbedding(fact);
   ...
   */
  async extractAndStoreEntities(fact: string, memoryId: string) {
    const prompt = [
      {
        role: 'system',
...
如果无法提取，返回空数组 []。`
   ...
   * 为指定对话生成精华摘要并存入情景记忆库
   */
  async generateEpisodeSummary(conversationId: string) {
    const history = this.memoryStore.getConversationHistory(conversationId);
    if (!history || history.length < 4) return; // 对话太短不值得摘要

    // 取最近 20 条消息进行摘要
    const recentMessages = history.slice(-20);
    const transcript = recentMessages.map((m: any) =>
      `${m.role === 'user' ? '用户' : 'AI'}：${typeof m.content === 'string' ? m.content.slice(0, 300) : '[多模态内容]'}`
    ).join('\n');
...
严格以 JSON 格式输出：{"summary": "摘要内容", "topics": ["主题1", "主题2"]}`
    ...
    try {
      ...
      if (result.summary) {
        ...
        if (embedding && Array.isArray(embedding) && embedding.length === EXPECTED_EMBEDDING_DIM) {
          await vectorDbManager.executeWrite(dbPath, async (episodeVecStore: any) => {
   ...
   */
  async unifiedRetrieval(query: string, knowledgeCategory: string = '默认知识库'): Promise<Array<{ content: string, source: string, score: number }>> {
    const { vectorDbManager } = require('./vector-db-manager');
    ...
    try {
      ...
      if (emb && Array.isArray(emb) && emb.length === EXPECTED_EMBEDDING_DIM) {
        queryEmbedding = emb;
      }
    } catch (e) {
            ...
            if (!memoryId) {
              try {
                ...
                if (matches && matches.length > 0 && matches[0].values.length > 0) {
                  ...
                  // 异步写回向量存储库存盘
                  setImmediate(async () => {
                    try { await vectorDbManager.executeWrite(dbPath, () => {}); } catch (e) {}
                  });
    ...
    tasks.push((async () => {
      try {
        ...
        await vectorDbManager.executeRead(dbPath, async (kbStore: any) => {
   ...
   */
  async processMemoryExtractionAsync(reply: string) {
  ...
  async cleanupOrphanVectors(): Promise<number> {
    ...
    try {
      ...
      const valid = data.filter((d: any) =>
        d && Array.isArray(d.embedding) && d.embedding.length === EXPECTED_EMBEDDING_DIM
      );
      const orphans = data.length - valid.length;
      if (orphans > 0) {
        fs.writeFileSync(dbPath, JSON.stringify(valid), 'utf8');
        console.warn(`[记忆引擎] 🧹 启动时清理 ${orphans} 条维度异常的孤儿向量（维度 != ${EXPECTED_EMBEDDING_DIM}），已触发重算/删除。`);
      }
      return orphans;
    } catch (e: any) {
