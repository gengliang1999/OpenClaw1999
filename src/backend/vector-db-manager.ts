...
export class VectorDatabaseManager {
  private static instance: VectorDatabaseManager;
  ...
  public static getInstance(): VectorDatabaseManager {
   ...
   */
  public async getStore(dbPath: string): Promise<VectorStore> {
   ...
   */
  public async executeWrite<T>(dbPath: string, operation: (store: VectorStore) => Promise<T> | T): Promise<T> {
   ...
   */
  public async executeRead<T>(dbPath: string, operation: (store: VectorStore) => Promise<T> | T): Promise<T> {
    const store = await this.getStore(dbPath);
   ...
   * 主动清理内存中的句柄
   */
  public clearCache(dbPath: string) {
    this.storeCache.delete(dbPath);
  }
}

export const vectorDbManager = VectorDatabaseManager.getInstance();
