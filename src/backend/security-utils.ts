...
/**
 * 安全工具集合（后端进程）
 *  - S7：命令指纹规范化（normalizeCommandFingerprint / matchFingerprint / isScopeAllowed）
 *  - I6：日志脱敏（redactForLog）
 * 全部为纯函数，无副作用，便于单元测试。
 */

export interface PermissionScopeLike {
 ...
 */
export function normalizeCommandFingerprint(cmd: string): string {
  if (!cmd) return '';
 ...
 * 绝不使用正则，杜绝元字符误匹配。
 */
export function matchFingerprint(stored: string, cmd: string): boolean {
  if (!stored || !cmd) return false;
  if (stored === cmd) return true;
  if (cmd.startsWith(stored + ' ')) return true;
  return false;
}
...
export function isScopeAllowed(
  scope: PermissionScopeLike | undefined,
  riskLevel?: 'high' | 'medium' | 'low',
  cwd?: string
): boolean {
  if (!scope) return true;
  if (scope.riskLevels && scope.riskLevels.length > 0) {
    if (riskLevel && !scope.riskLevels.includes(riskLevel)) return false;
  }
  if (scope.cwd && cwd) {
    if (cwd !== scope.cwd && !cwd.startsWith(scope.cwd + '/')) return false;
  }
  return true;
}
...
export function redactForLog(line: string): string {
  ...
  for (const re of SENSITIVE_KEY_RE) {
    s = s.replace(re, (m: string, p1?: string) => {
