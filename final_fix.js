...
rl.on('close', () => {
    ...
    if (typeof content === 'string' && content.startsWith('"')) {
        try {
            ...
            console.log('JSON parse failed, manual unescape', e.message);
    ...
    // Since we know the previous replace caused literal newlines, let's just make sure it's fully unescaped:
    content = content.replace(/\\n/g, '\n').replace(/\\"/g, '"').replace(/\\\\/g, '\\');
